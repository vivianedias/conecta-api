...
class FormularioProjeto extends Component {
	...
	nextStepValidation = () => {
		...
		if(step === 0) {
			if(!description || !objective) {
				!description && 
				this.validateBeforeNext(description, 'description');
				!objective && 
				this.validateBeforeNext(objective, 'objective');
			}
		...
		if(step === 1) {
			...
			!format || format === '' 
				? this.validateBeforeNext(format, 'format') 
				: this.nextStep();
		} 
		...
		if(step === 2) {
			if((typeof tags !== 'undefined' && tags.length === 0) || !location || !estimatedValue ) {
				!location && 
				this.validateBeforeNext(location, 'location');
				!estimatedValue && 
				this.validateBeforeNext(estimatedValue, 'estimatedValue');
				typeof tags !== 'undefined' && tags.length === 0 && 
				this.validateBeforeNext(tags, 'tags');
			}
	...
	validateBeforeNext = (value, name) => {
		const { handleErrors } = this.props;
		const validationErrors = utilsService.validate(value, name);
	...
	// Submit form
	onSubmit = (e) => {
		const { projectForm, sendForm, errors } = this.props;
		e.preventDefault();

		if(!projectForm.handle || !projectForm.img ) {
			!projectForm.handle &&
			this.validateBeforeNext(projectForm.handle, 'handle');
			!projectForm.img &&
			this.validateBeforeNext(projectForm.img, 'img');
		}
