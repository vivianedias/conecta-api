...
class Projetos extends Component { 
    ...
    handleChange = () => {
			...
			(item.category && item.category.includes(inputValue)) || 
			(item.handle && item.handle.includes(inputValue)) || 
			(item.description && item.description.includes(inputValue)) ||
			(item.objective && item.objective.includes(inputValue)) || 
			(item.format && item.format.includes(inputValue)) ||
		...
		return (
			<div className="projetos-comp">
				<div className="projetos-comp__wrapper">
					<h1 
						className="projetos-comp__h1 title is-1"
					>
						Projetos_
					</h1>
						...
						{projects && projects.length >= 5 && (
							...
							>
								{projects && projects.length === this.state.cardSlice 
									? 'Ver menos' 
									: 'Ver mais'
								}
							</button>
